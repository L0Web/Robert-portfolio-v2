...
export default function ImageViewer({ images, ref, closeDialog }: ImageViewerProps) {
    ...
    return (
        <dialog ref={ref} className="fixed z-[999] top-0 left-0 flex pointer-events-none open:pointer-events-auto opacity-0 open:opacity-100 w-screen h-screen bg-white/50 backdrop-blur-lg backdrop:bg-transparent transition-opacity duration-500 ease-expo">
            <div className="relative w-full h-full p-6 grid grid-cols-[1fr,_4fr,_1fr] gap-4">
                <div className="flex">
                    <button onClick={closeDialog} className="flex items-center justify-center gap-2 pl-4 pr-2 h-8 border border-gray-400 text-gray-600 rounded-full hover:bg-black/10 hover:border-transparent">
                ...
                <div className="flex flex-col justify-between gap-4 items-end">
                    <span className="flex items-center justify-center gap-2 text-xl tracking-tight font-geist text-gray-600">
                    ...
                    <div className="flex items-center justify-center gap-2">
                        <button onClick={prevImage} className="h-8 rounded-full px-4 flex items-center justify-center border border-gray-400 hover:bg-black/20 hover:border-transparent">
                            <GoArrowLeft size={18} />
                        </button>
                        <button onClick={nextImage} className="h-8 rounded-full px-4 flex items-center justify-center border border-gray-400 hover:bg-black/20 hover:border-transparent">
