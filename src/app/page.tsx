    ...
    <FetchHandler loading={loading} error={error} refetch={refetch}>
      <div className="group font-[family-name:var(--font-geist-sans)] m-auto max-w-[var(--max-width)]">
        <section className="lg:mt-[20vh] min-h-[30vh] flex flex-col gap-10 justify-center items-center">
          <p className="text-gray-600 text-center w-full flex-wrap max-w-[45ch] lg:max-w-[60ch] text-xl leading-tight lg:text-2xl">
            {
            ...
            <div className="absolute p-4 top-0 left-0 w-full h-[480px] overflow-hidden lg:hover:overflow-x-auto flex items-center justify-center gap-4">
              {
                data?.artworks?.slice(0, 5)?.map(({ id, title, images, categories }) => (
                  <Link href={`/artworks/artwork/${id}`} key={title} className="group/child-1 relative flex flex-col">
                    ...
                    <div className="flex justify-between items-center gap-2 mt-2">
                      <h3 className="opacity-0 group-hover/child-1:opacity-100 transition-opacity duration-300 ease-expo text-sm tracking-tighter font-semibold text-black/80 overflow-hidden">{title}</h3>
                      <span className="opacity-0 group-hover/child-1:opacity-100 transition-opacity duration-300 ease-expo text-[.6rem] tracking-tight flex items-center justify-center px-2 h-4 rounded-full bg-black/20">Details</span>
                    </div>
                    <span className="block overflow-hidden">
                      <span className="text-[.75rem] text-black/60 block translate-y-full group-hover/child-1:translate-y-0 transition-transform duration-500 ease-expo">
                        {
